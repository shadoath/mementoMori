...
function spawn_year(_year, birthday) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    for (let j = 0; j < 4; j++) {
      ...
      if (
        new Date(_year, i, Math.floor((j + 1) * num_days_per_square)) < birthday
      ) {
...
function write_life_event(life_event) {
  ...
  if (week_div == null || week_div.classList.contains('invisible')) {
    ...
    console.error(
      `Event "${life_event['description']}" has an invalid date (${y}-${m}-${d})`
    );
    return;
  }
  ...
  if (
    life_event['description'] != null &&
    life_event['description'].length > 0
  ) {
